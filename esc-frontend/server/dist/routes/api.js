...
const decodeVoteStateCookie = (cookieValue) => {
    try {
...
const juryPointValues = [1, 2, 3, 4, 5, 6, 7, 8, 10, 12];
...
apiRouter.post("/auth/login", authLimiter, async (req, res) => {
    ...
    if (isMockMode()) {
        const isValid = (role === "admin" && password === "admin-token") ||
...
apiRouter.get("/contest/current", async (_req, res) => {
    ...
    if (raw && typeof raw === "object" && "songId" in raw) {
        const { runId, currentIndex, totalSongs, songId, songName, youtubeUrl, countryId, countryName, artistId, artistFirstName, artistLastName, artistType, publicVotes, juryVotes, totalVotes, votingIsOpen, ...rest } = raw;
        res.json({
            payload: {
...
apiRouter.get("/vote/state", (req, res) => {
    ...
    if (!state) {
        const raw = req.cookies?.vote_state;
        if (raw) {
...
apiRouter.post("/vote", async (req, res) => {
    ...
    res.status(response.status).json(response.status === 201
...
apiRouter.post("/jury/vote", authorizedLimiter, requireRole("jury"), async (req, res) => {
    ...
    if (!songID || !juryPointValues.includes(points)) {
...
apiRouter.get("/admin/authenticate", authLimiter, async (req, res) => {
    ...
    if (isMockMode()) {
        res.status(token === "admin-token" ? 202 : 403).json(token === "admin-token" ? { message: "ok" } : { error: "Invalid admin token" });
        return;
    }
...
apiRouter.get("/jury/authenticate", authLimiter, async (req, res) => {
    const token = String(req.query.Token ?? "");
    if (isMockMode()) {
        res.status(token === "jury-token" ? 202 : 403).json(token === "jury-token" ? { message: "ok" } : { error: "Invalid jury token" });
        return;
    }
...
apiRouter.get("/results", async (_req, res) => {
    ...
    const juryPayload = (juryResponse.data?.payload ?? []);
    juryPayload.forEach((entry) => juryMap.set(entry.id, entry.juryVotes ?? 0));
    const results = (escResponse.data?.payload ?? []).map((song) => {
...
apiRouter.get("/stats", async (_req, res) => {
    ...
    const votes = (response.data?.payload ?? []);
